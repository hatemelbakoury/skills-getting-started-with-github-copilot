      ...
      Object.entries(activities).forEach(([name, details]) => {
        ...
        activityCard.innerHTML = `
          ...
          <div class="participants">
            ...
            <ul>
              ${details.participants.length > 0 ? details.participants.map(p => `<li><span class="participant-name">${p}</span><button class="delete-btn" data-activity="${name}" data-participant="${p}" title="Unregister ${p}">×</button></li>`).join('') : '<li>No participants yet</li>'}
            </ul>
          </div>
        `;
